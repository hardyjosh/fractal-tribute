...
onMount(() => {
  if (creator === undefined) {
    throw new Error(`The creator input is required for the CreateEvmKeyBinding element`);
  }
});
...
async function createEvmKeyBinding() {  
  ...
  try {
    ...
    dispatch('evm-key-binding-created', { evmKeyBindingHash: record.signed_action.hashed.hash });
  } catch (e) {
